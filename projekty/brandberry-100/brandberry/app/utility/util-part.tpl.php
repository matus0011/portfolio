...
if ( ! function_exists( 'brandberry_post_thumbnail' ) ) :
	
	function brandberry_post_thumbnail() {
		...
		<?php else : ?>
			<div class="blog__details-top post-format-<?php echo esc_attr(get_post_format()); ?> <?php echo esc_attr(get_post_format() == 'video' || is_sticky() ? 'position-relative' : ''); ?> ">
			    <!-- Video format -->
			    <?php if(get_post_format() == 'video' && brandberry_meta_option(get_the_ID(),'feature_video_id') !=''): ?>
					<div class="joya-video-btn">
			            <a href="<?php echo brandberry_meta_option(get_the_ID(),'feature_video_id'); ?>" class="play-btn video-popup"><i class="icon-wcf-play-fill"></i></a>
			        </div>
			    <?php endif; ?>
			    <!-- Audio Format -->
			    <?php if(get_post_format() == 'audio' && brandberry_meta_option(get_the_ID(),'feature_audio') !=''): ?>
			        <div class="joya-blog-audio">
		              <?php echo wp_oembed_get(brandberry_meta_option(get_the_ID(),'feature_audio')); ?>
		            </div>
			    <?php endif; ?>
			    <!-- Sticky Post ribbon -->
		        <?php 
...
if ( ! function_exists( 'brandberry_return' ) ) :

  function brandberry_return($arg){ 
   return $arg;
  }
...
// ----------------------------------------------------------------------------------------
if ( !function_exists('brandberry_get_breadcrumbs') ) {
   
	function brandberry_get_breadcrumbs( $seperator = '/', $word = '' ) {

		$general_breadcrumb_limit = brandberry_option('general_breadcrumb_limit');
		$breadcrumb_enable        = brandberry_option('breadcrumb_enable',1);
            $general_custom_post_type = brandberry_option('general_custom_post_type');
		...
		if ( !is_home() ) {
			...
			echo '">';
			echo esc_html__( 'Home', 'brandberry' ) . wp_kses_post( $seperator );
			echo "</a></li> ";
			...
			echo '<li class="active">';
			 echo esc_html__('Shop', 'brandberry');
			echo '</li>';
		    }
			elseif( is_page() ) {
		...
		if ( is_tag() ) {
			...
			echo"<li class='active'>" . esc_html__( 'Blogs for', 'brandberry' ) . " ";
		    	the_time( 'F jS, Y' );
			echo'</li>';
			...
			echo"<li class='active'>" . esc_html__( 'Author Blogs', 'brandberry' );
			echo'</li>';

		} elseif ( isset( $_GET[ 'paged' ] ) && !empty( $_GET[ 'paged' ] ) ) {

			echo "<li class='active'>" . esc_html__( 'Blogs', 'brandberry' );
			echo'</li>';

		} elseif ( is_search() ) {

			echo"<li class='active'>" . esc_html__( 'Search Result', 'brandberry' );
			echo'</li>';

		} elseif ( is_404() ) {
		
			$_404_banner_title  = esc_html__( '404 Error', 'brandberry' );
...
if(!function_exists('brandberry_has_post_tags')) {
	function brandberry_has_post_tags(){
		$single_post_tags = brandberry_option('single_post_tags','1');
	 	if( ! $single_post_tags ){
		   return;
		}
...
/*-----------------------------
	RANDOM SINGLE TAG
------------------------------*/
if ( !function_exists( 'brandberry_random_tag_retrip' ) ): 
	function brandberry_random_tag_retrip(){
