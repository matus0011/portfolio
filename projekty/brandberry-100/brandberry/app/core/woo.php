...
class Wcf_Woo {
	...
	public function  _output_related_products_args($args) {
		if( brandberry_option('wcf_rel_product_cols') != '' && is_numeric(brandberry_option('wcf_rel_product_cols'))) {
			$args['posts_per_page'] = brandberry_option('wcf_rel_product_cols');
			$args['columns'] = brandberry_option('wcf_rel_product_cols');
		}
		return $args;
	}
