(function ($) {
  window.addEventListener('elementor/frontend/init', function () {
    class WCFNestedSliderHandler extends elementorModules.frontend.handlers.Base {
      ...
      updateSliderOnOptionChange(propertyName) {
        ...
        async function inlineSvg(imgEl) {
          try {
            ...
            const response = await fetch(imgEl, {
              mode: 'cors'
            });
      ...
      run() {
        ...
        const {
          ...
          options,
          slider_exist
        } = thisModule.getSliderOptions(thisModule.$element);
        if (slider_exist) {
          const nestedSlider = new elementorFrontend.utils.swiper(slider, options).then(newSwiperInstance => {
    ...
    const widgets = ['wcf--nested-slider.default'];
    widgets.forEach(widgetName => {
      elementorFrontend.hooks.addAction(`frontend/element_ready/${widgetName}`, $element => {
        elementorFrontend.elementsHandler.addHandler(WCFNestedSliderHandler, {
          $element
        });
      });
    });
