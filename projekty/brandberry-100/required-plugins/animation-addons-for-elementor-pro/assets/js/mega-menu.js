  ...
  window.addEventListener('elementor/frontend/init', function () {
    class MegaMenu extends elementorModules.frontend.handlers.Base {
      ...
      setContentContainerAbsolutePosition($contentContainer) {
        ...
        if (isMenuInDropdownMode(elementSettings)) {
          return;
        }
        if (isFitToContent) {
      ...
      onInit() {
        this.menuHeightController = new DropdownMenuHeightController(this.dropdownMenuHeightControllerConfig());
      ...
      menuToggleVisibilityListener($menuToggle) {
        let previousWidth;
        const selectors = this.getSettings('selectors');
        let wwa = this.findElement(selectors.menuWrapper);
        this.observedContainer = new ResizeObserver(menuToggle => {
          ...
          if (currentWidth !== previousWidth) {
            ...
            // if(device == 'mobile'){
            //   wwa[0].style.setProperty('--n-menu-dropdown-content-box-height','0');
            // }else{
            //   wwa[0].style.setProperty('--n-menu-dropdown-content-box-height','auto');
            // }

            // wwa[0].style.setProperty('--stretch-width', currentWidth+'px');
            // wwa[0].style.setProperty('--stretch-left', '-10px');
          }
        });
        this.observedContainer.observe(document.querySelector('body'));
      }
      ...
      setLayoutType() {
        let layoutType = 'none' === this.elements.$dropdownMenuToggle.css('display') ? 'horizontal' : 'dropdown';
        const device = elementorFrontend.getCurrentDeviceMode();
        if ('mobile' === device) {
          layoutType = 'dropdown';
          this.elements.$dropdownMenuToggle.css('display', 'flex');
          this.elements.$widgetContainer.find('.e-n-menu-wrapper')[0].style.setProperty('--n-menu-dropdown-content-box-height', '10');
        }
      ...
      changeScrollStatusAndDispatch(navigationWrapper, event) {
        changeScrollStatus(navigationWrapper, event);
      }
      setHorizontalTitleScrollValuesAndDispatch(navigationWrapper, event) {
        setHorizontalTitleScrollValues(navigationWrapper, this.getHorizontalScrollSetting(), event);
      }
      linkContainer(event) {
        const {
            container
    ...
    elementorFrontend.hooks.addAction(`frontend/element_ready/aae-nested-mega-menu.default`, $element => {
