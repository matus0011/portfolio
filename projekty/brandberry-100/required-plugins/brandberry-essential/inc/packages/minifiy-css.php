...
abstract class brandberry_MinificationSequenceFinder
{
    public $start_idx;
    public $end_idx;
    public $type;
...
class brandberry_StringSequenceFinder extends brandberry_MinificationSequenceFinder
{
...
class brandberry_QuoteSequenceFinder extends brandberry_MinificationSequenceFinder
{
...
$singlebrandberry_QuoteSequenceFinder = new brandberry_QuoteSequenceFinder('\'');
$doublebrandberry_QuoteSequenceFinder = new brandberry_QuoteSequenceFinder('"');
$blockCommentFinder = new brandberry_StringSequenceFinder('/*', '*/');
$minificationStore = array();

function brandberry_getNextMinificationPlaceholder(){
    global $minificationStore;
    return '<-!!-' . sizeof($minificationStore) . '-!!->';
}
function brandberry_getNextSpecialSequence($string, $sequences){
...
function brandberry_minify_CSS($css){
    global $minificationStore, $singlebrandberry_QuoteSequenceFinder, $doublebrandberry_QuoteSequenceFinder, $blockCommentFinder;
    $css_special_chars = array($blockCommentFinder, // CSS Comment
    $singlebrandberry_QuoteSequenceFinder, // single quote escape, e.g. :before{ content: '-';}
    $doublebrandberry_QuoteSequenceFinder); // double quote
    // pull out everything that needs to be pulled out and saved
    while ($sequence = brandberry_getNextSpecialSequence($css, $css_special_chars)){
        switch ($sequence->type){
        case '/*': // remove comments
        $css = substr($css, 0, $sequence->start_idx) . substr($css, $sequence->end_idx);
        break;
        default: // strings that need to be preservered
        $placeholder = brandberry_getNextMinificationPlaceholder();
