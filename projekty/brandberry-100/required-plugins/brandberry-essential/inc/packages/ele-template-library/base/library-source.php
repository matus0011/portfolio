...
class Library_Source extends Source_Base {
	...
	public function get_id() {
		return 'wcf-theme-layout-manager';
	}
	...
	public function get_file_info( $template_id ) {
		if ( empty( $template_id ) ) {
			return;
		}	
		$request_url = BRANDBERRY_ESSENTIAL_ELEMENTOR_SECTION_PATH . $template_id;
	...
	public function request_template_data( $template_id ) {
		...
		$file_data = $this->get_file_info( $template_id );
	
		if ( empty( $file_data ) ) {
			return;
		}			
		$file_array = json_decode( $file_data, true ); 
		if ( empty( $file_array ) || ! isset( $file_array['url'] ) ) {
			return;
		}
