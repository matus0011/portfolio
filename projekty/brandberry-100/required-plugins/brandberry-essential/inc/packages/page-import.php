...
final class BandBerry_Admin_Page_Importer
{

    const HANDLE = 'brandberry-admin-actions';
    ...
    public function clear_notices_for_importer()
    {
        ...
        if ($screen && ($screen->id === 'admin_page_brandberry-page-importer' || $screen->id === 'brandberry-theme_page_brandberry-page-importer')) {
    ...
    public function add_menu()
    {
        ...
        add_submenu_page(
            BRANDBERRY_ESSENTIAL_THEME_PARENT_SLUG,                 // 👈 null keeps it hidden from UI
            'Page Import',          // Page title
            'Page Import',          // Menu title (ignored since it's hidden)
            'manage_options',       // Capability
            'brandberry-page-importer',       // Slug
    ...
    public function enqueue($hook_suffix)
    {
        ...
        wp_register_script(
            self::HANDLE,
            BRANDBERRY_ESSENTIAL_URL . 'assets/js/aae-admin-actions.min.js',
            ['jquery'],
            time(),
            true
        );   
        ...
        wp_enqueue_style(
            self::HANDLE,
            BRANDBERRY_ESSENTIAL_URL . 'assets/css/aae-admin-actions.css',
            [],
            time()
        );
    
        wp_localize_script(self::HANDLE, 'BRANDBERRY_PAGE_IMPORT', [
            'nonce'   => wp_create_nonce('aae_admin_nonce'),
            'screen'  => $screen->id,
            'post_id' => $post_id,
            'logo' => file_exists(BRANDBERRY_ESSENTIAL_URL . 'assets/images/wcf-2.png') ? BRANDBERRY_ESSENTIAL_URL . 'assets/images/wcf-2.png' : null,
            'page_url'   => esc_url(admin_url('admin.php?page=brandberry-page-importer')),
        ]);
    ...
    public function importer_assets($hook)
    {
        ...
        if ($hook == 'admin_page_brandberry-page-importer' || $hook == 'brandberry-theme_page_brandberry-page-importer') {
            // CSS
            wp_enqueue_style(
                'brandberry-page-importer-admin', // Handle for the stylesheet
                BRANDBERRY_ESSENTIAL_URL . 'assets/build/modules/page-import/index.css',
                array(), // Dependencies (none in this case)
                time()
            );
            
            wp_enqueue_script('brandberry-page-importer-admin', BRANDBERRY_ESSENTIAL_URL . 'assets/build/modules/page-import/index.js', array('wp-element', 'wp-i18n'), time(), true);
            $localize_data = array(
                ...
                'user_role'           => function_exists('wcfaddon_get_current_user_roles') ? wcfaddon_get_current_user_roles() : [],
                'version'             => time(),
                'st_template_domain'  => BRANDBERRY_ESSENTIAL_ELEMENTOR_SECTION_BASE,
                'theme_name'      => BRANDBERRY_TPL_SLUG,
                'home_url'            => home_url('/'),
            );
           
            wp_localize_script('brandberry-page-importer-admin', 'BRANDBERRY_THEME_ADMIN', $localize_data);
        }
    }
}
if (is_admin()) {
    new BandBerry_Admin_Page_Importer();
}
