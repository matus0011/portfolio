...
namespace BrandberryEssentialApp\Traits;
...
trait BB_Post_Query_Trait {
	...
	protected function register_query_controls() {
		$this->start_controls_section(
			'section_query',
			[
				'label' => esc_html__( 'Query', 'brandberry' ),
		...
		$this->add_control(
			'query_type',
			[
				'label'   => esc_html__( 'Query Type', 'brandberry' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'custom',
				'options' => apply_filters( 'bb_widget_wp_query_type', [
					'custom'  => esc_html__( 'Custom', 'brandberry' ),
					'archive' => esc_html__( 'Archive', 'brandberry' ),
					'related' => esc_html__( 'Related', 'brandberry' )
				] ),
			]
		);
		...
		$this->add_control(
			'post_type',
			[
				'label'     => esc_html__( 'Source', 'brandberry' ),
				'type'      => Controls_Manager::SELECT,
				'default'   => 'post',
		...
		$this->start_controls_tab(
			'query_include',
			[
				'label'     => esc_html__( 'Include', 'brandberry' ),
		...
		$this->add_control(
			'include',
			[
				'label'       => esc_html__( 'Include By', 'brandberry' ),
				'type'        => Controls_Manager::SELECT2,
				'label_block' => true,
				'multiple'    => true,
				'options'     => [
					'terms'   => esc_html__( 'Term', 'brandberry' ),
					'authors' => esc_html__( 'Author', 'brandberry' ),
				],
				'condition'   => [ 'query_type' => 'custom' ],
			]
		);
		...
		$this->add_control(
			'include_term_ids',
			[
				'label'       => esc_html__( 'Term', 'brandberry' ),
				'description' => esc_html__( 'Add coma separated, terms id', 'brandberry' ),
				'placeholder' => esc_html__( 'All', 'brandberry' ),
				'label_block' => true,
				'ai'          => [
					'active' => false,
				],
		...
		$this->add_control(
			'include_authors',
			[
				'label'       => esc_html__( 'Author', 'brandberry' ),
				'description' => esc_html__( 'Add separated, authors ID', 'brandberry' ),
				'placeholder' => esc_html__( 'All', 'brandberry' ),
				'label_block' => true,
				'ai'          => [
					'active' => false,
		...
		$this->start_controls_tab(
			'query_exclude',
			[
				'label'     => esc_html__( 'Exclude', 'brandberry' ),
				'condition' => [ 'query_type' => 'custom' ],
			]
		);
		...
		$this->add_control(
			'exclude',
			[
				'label'       => esc_html__( 'Exclude By', 'brandberry' ),
				'type'        => Controls_Manager::SELECT2,
				'label_block' => true,
				'multiple'    => true,
				'options'     => [
		...
		$this->add_control(
			'post_format',
			[
				'label'     => esc_html__( 'Post Format', 'brandberry' ),
				'type'      => Controls_Manager::SELECT2,
				'default'   => [],
				'multiple'  => true,
				'options'   => [
					'post-format-image'   => esc_html__( 'Image', 'brandberry' ),
					'post-format-video'   => esc_html__( 'Video', 'brandberry' ),
					'post-format-audio'   => esc_html__( 'Audio', 'brandberry' ),
					'post-format-gallery' => esc_html__( 'Gallery', 'brandberry' ),
				],
				'condition' => [ 'query_type' => [ 'custom' ] ],
			]
		);
		...
		$this->add_control(
			'post_categories',
			[
				'label'     => esc_html__( 'Post Categories', 'brandberry' ),
				'type'      => Controls_Manager::SELECT2,
				'default'   => [],
				'multiple'  => true,
		...
		$this->add_control(
			'post_tags',
			[
				'label'     => esc_html__( 'Post Tags', 'brandberry' ),
				'type'      => Controls_Manager::SELECT2,
				'default'   => [],
				'multiple'  => true,
		...
		$this->add_control(
			...
			[
				...
				'default'   => 'anytime',
				'options'   => [
					'anytime'  => esc_html__( 'All', 'brandberry' ),
					'-1 day'   => esc_html__( 'Past Day', 'brandberry' ),
					'-3 day'   => esc_html__( 'Past 3 Day', 'brandberry' ),
					'-1 week'  => esc_html__( 'Past Week', 'brandberry' ),
					'-2 week'  => esc_html__( 'Past Two Weeks', 'brandberry' ),
					'-1 month' => esc_html__( 'Past Month', 'brandberry' ),
					'-3 month' => esc_html__( 'Past Quarter', 'brandberry' ),
					'-1 year'  => esc_html__( 'Past Year', 'brandberry' ),
				],
				'condition' => [
					'query_type' => [
						'custom',
		...
		$this->add_control(
			'post_order_by',
			[
				'label'     => esc_html__( 'Order By', 'brandberry' ),
				'type'      => Controls_Manager::SELECT,
				'default'   => 'date',
				'options'   => [
					'date'          => esc_html__( 'Date', 'brandberry' ),
					'title'         => esc_html__( 'Title', 'brandberry' ),
					'menu_order'    => esc_html__( 'Menu Order', 'brandberry' ),
					'modified'      => esc_html__( 'Last Modified', 'brandberry' ),
					'comment_count' => esc_html__( 'Comment Count', 'brandberry' ),
					'rand'          => esc_html__( 'Random', 'brandberry' ),
				],
				'condition' => [ 'query_type' => 'custom' ],
			]
		);
		...
		$this->add_control(
			'post_order',
			[
				'label'     => esc_html__( 'Order', 'brandberry' ),
				'type'      => Controls_Manager::SELECT,
				'default'   => 'desc',
				'options'   => [
					'asc'  => esc_html__( 'ASC', 'brandberry' ),
					'desc' => esc_html__( 'DESC', 'brandberry' ),
				],
				'condition' => [ 'query_type' => 'custom' ],
			]
		);
		...
		$this->add_control(
			'post_sticky_ignore',
			[
				'label'        => esc_html__( 'Ignore Sticky Posts', 'brandberry' ),
				'type'         => Controls_Manager::SWITCHER,
				'label_on'     => esc_html__( 'Yes', 'brandberry' ),
				'label_off'    => esc_html__( 'No', 'brandberry' ),
				'return_value' => 'yes',
				'default'      => 'yes',
				'condition'    => [ 'query_type' => 'custom' ],
			]
