...
class Wcf_Dynamic_Cpt extends Custom_Post
{
    ...
    function isearch_filter_post_type($query) {
        ...
        if ( ! is_admin() && $query->is_main_query() ) {
            ...
            $search_result_post_types = brandberry_option('search_result_post_types', false);            
    ...
    public function create_taxonomy(){
        ...
        $cpt_options = get_option(BRANDBERRY_ESSENTIAL_OPTION_KEY);
        if( isset( $cpt_options[ 'cpt_taxonomy_options' ] ) && is_array($cpt_options[ 'cpt_taxonomy_options' ]) && count($cpt_options[ 'cpt_taxonomy_options' ]) > 0){
            ...
            foreach($options as $option){
                ...
                if($option['taxonomy_name']  != '' && isset($option['post_types'])){
                    ...
                    $labels = array(
                        'name'              => _x( $option['taxonomy_plural_label'], $option['taxonomy_plural_label'], 'brandberry-essential' ),
                        'singular_name'     => _x( $option['taxonomy_label'], $option['taxonomy_label'] , 'brandberry-essential' ),
                        'search_items'      => __( "Search $plural_label", 'brandberry-essential' ),
                        'all_items'         => __( "All $plural_label", 'brandberry-essential' ),
                        'parent_item'       => __( "Parent $singular_label ", 'brandberry-essential' ),
                        'parent_item_colon' => __( "Parent $singular_label :", 'brandberry-essential' ),
                        'edit_item'         => __( "Edit $singular_label", 'brandberry-essential' ),
                        'update_item'       => __( "Update $singular_label ", 'brandberry-essential' ),
                        'add_new_item'      => __( "Add New $singular_label ", 'brandberry-essential' ),
                        'new_item_name'     => __( "New $singular_label ", 'brandberry-essential' ),
                        'menu_name'         => __( $singular_label , 'brandberry-essential' ),
                    );
