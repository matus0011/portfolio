(function ($) {
    ...
    const getSliderOptions = function ($scope) {
        const slider = $($('.bb__slider', $scope)[0]);
      
        const slexist = $scope.find('.bb__slider').length;
        const sliderSettings = $($('.bb__slider-wrapper, .bb__t_slider-wrapper', $scope)[0]).data('settings') || {};
        sliderSettings.handleElementorBreakpoints = true
      
        //navigation
        if (sliderSettings.hasOwnProperty('navigation')) {
            const next = $('.bb-arrow-next', $scope)[$('.bb-arrow-next', $scope).length - 1]
            const prev = $('.bb-arrow-prev', $scope)[$('.bb-arrow-prev', $scope).length - 1]
        ...
        $($('.bb__slider-wrapper', $scope)[0]).removeAttr('data-settings');

        return {slider: slider, options: sliderSettings, slider_exist: slexist};
    }

    const getThumbSliderOptions = function ($scope) {
        const slider = $('.bb__thumb_slider', $scope);
        const sliderSettings = $('.bb__thumb-slider-wrapper', $scope).data('settings') || {};
        sliderSettings.handleElementorBreakpoints = true

        //remove the attribute after getting the slider settings
        $('.bb__thumb-slider-wrapper', $scope).removeAttr('data-settings');
    ...
    $(window).on('elementor/frontend/init', function () {
        ...
        $.each(WcfSliderWidgets, function (widget, $skins) {
            elementorFrontend.hooks.addAction(`frontend/element_ready/bb--${widget}.default`, Slider);        
            if ($skins.length) {
                for (const $skin of $skins) {
                    elementorFrontend.hooks.addAction(`frontend/element_ready/bb--${widget}.${$skin}`, Slider);
                }
            }
        });
